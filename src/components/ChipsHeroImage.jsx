    ...
    >
      {
        chipData?.map(item => (
          ...
          null
          :
          <ListItem key={item?.image?.name} sx={{display: 'flex', flexWrap: 'wrap', alignItems: 'center'}}>
            <Chip
              avatar={<Avatar alt={item?.image?.name} src={item?.imageUrl} />}
              label={item?.image?.name.slice(0, 15)}
              onDelete={handleDelete(item)}
            />
            <Typography sx={{width: '100%'}}>{`veličina slike: ${chipData && Math.ceil(item?.image?.size/1000) + "kb"}`}</Typography>
          </ListItem>
        ))
      }
    </Paper>
